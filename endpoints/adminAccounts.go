...
func HandleUserSignInProcess(ctx *gin.Context) {
	...
	err := db.DB.QueryRow("SELECT id, phone_number, password, salt, name FROM admin_account WHERE phone_number = $1 LIMIT 1",
		requestBody.Phone).Scan(&storedAccount.ID, &storedAccount.PhoneNumber, &storedAccount.Password, &storedAccount.Salt, &storedAccount.Name)
	...
	ctx.JSON(http.StatusOK, gin.H{"status": true, "message": "Sign-in successful", "value": storedAccount.ID})
}
func HandleGettingUserAccount(ctx *gin.Context) {
	...
	err := db.DB.Select(&Acocount, "SELECT id, name, phone_number FROM account WHERE id = $1 LIMIT 1", requestBody.ID)
